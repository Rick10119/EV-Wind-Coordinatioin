...
stairs(value(P_C),'r','LineWidth',1); hold on;
stairs(value(P_w),'b--','LineWidth',2);ylim([-25 200]);
ha=gca;
%设置figure各个参数

t1 = title('有序充电模式','FontSize',24);
...
saveas(gcf,'有序充电.jpg'); %保存当前窗口的图像
zacconut=[zacconut,value(Z_CD)];
zuacconut=[zuacconut,value(Z_u)];%记录总成本
zgiveup=[zgiveup,100*sum(P_w_max-value(P_w))/sum(P_w_max)];%弃风率
zshuju=[zshuju,value(P_C),value(sum(P_u,2)),value(P_w)];